...
import { GitBranch, Github, AlertCircle, CheckCircle, Clock, Settings, Plus, ExternalLink } from 'lucide-react';
...
export default function GitHubIntegrationPage() {
  ...
  const getStatusIcon = (status: ReviewSession['status']) => {
    switch (status) {
      case 'reviewed':
      case 'commented':
        return <CheckCircle className="h-4 w-4 text-green-500" data-testid="icon-success" />;
      case 'analyzing':
      case 'queued':
        return <Clock className="h-4 w-4 text-blue-500" data-testid="icon-processing" />;
      case 'error':
        return <AlertCircle className="h-4 w-4 text-red-500" data-testid="icon-error" />;
      default:
        return <Clock className="h-4 w-4 text-gray-500" data-testid="icon-default" />;
    }
  };
  ...
  return (
    <div className="container mx-auto p-6 space-y-6" data-testid="github-integration-page">
      <div className="flex items-center justify-between">
        ...
        <Button onClick={installGitHubApp} className="flex items-center gap-2" data-testid="button-install-app">
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-6">
        ...
        <TabsContent value="overview" className="space-y-6" data-testid="tab-content-overview">
              ...
              <CardContent className="p-6">
                <div className="flex items-center">
                  ...
                  <div className="ml-4">
                    ...
                    <p className="text-2xl font-bold text-gray-900 dark:text-white" data-testid="stat-repos">
              ...
              <CardContent className="p-6">
                <div className="flex items-center">
                  ...
                  <div className="ml-4">
                    ...
                    <p className="text-2xl font-bold text-gray-900 dark:text-white" data-testid="stat-active">
                      {mockReviewSessions.filter(r => ['analyzing', 'queued'].includes(r.status)).length}
                    </p>
                  </div>
                </div>
              </CardContent>
                ...
                <div className="flex items-center">
                  ...
                  <div className="ml-4">
                    ...
                    <p className="text-2xl font-bold text-gray-900 dark:text-white" data-testid="stat-completed">
                      {mockReviewSessions.filter(r => ['reviewed', 'commented'].includes(r.status)).length}
                    </p>
                  </div>
                </div>
              </CardContent>
          ...
          <Alert data-testid="alert-installation-info">
            ...
            <AlertDescription>
              Install the Mesrai AI GitHub App to start automated code reviews on your repositories.
              The app will automatically review new pull requests and provide detailed feedback.
            </AlertDescription>
          </Alert>
        </TabsContent>

        <TabsContent value="repositories" className="space-y-6" data-testid="tab-content-repositories">
          <Card>
            <CardHeader>
              <CardTitle className="flex items-center gap-2" data-testid="repos-title">
                ...
                <div className="space-y-4" data-testid="repos-list">
                  {mockRepositories.map((repo) => (
                    ...
                    >
                      <div className="flex items-center space-x-4">
                        ...
                        <div>
                          <div className="flex items-center gap-2">
                            <h3 className="text-lg font-medium text-gray-900 dark:text-white" data-testid={`repo-name-${repo.id}`}>
                              {repo.fullName}
                            </h3>
                            {repo.isPrivate && (
                              <Badge variant="secondary" data-testid={`repo-private-${repo.id}`}>
                                Private
                              </Badge>
                            )}
                            <Badge 
                              variant={repo.isActive ? "default" : "secondary"}
                              className={repo.isActive ? "bg-green-500/10 text-green-700 dark:text-green-400" : ""}
                      ...
                      <Button variant="outline" size="sm" data-testid={`button-configure-${repo.id}`}>
                  ...
                  {mockRepositories.length === 0 && (
                    <div className="text-center py-8" data-testid="repos-empty">
                      ...
                      <Button onClick={installGitHubApp} data-testid="button-install-from-empty">
                        Install GitHub App
                      </Button>
                    </div>
                  )}
        ...
        <TabsContent value="reviews" className="space-y-6" data-testid="tab-content-reviews">
          <Card>
            <CardHeader>
              <CardTitle className="flex items-center gap-2" data-testid="reviews-title">
            ...
            <CardContent>
              <div className="space-y-4" data-testid="reviews-list">
                {mockReviewSessions.map((review) => {
                  const repo = mockRepositories.find(r => r.id === review.repositoryId);
                  return (
                    <div
                      key={review.id}
                      className="flex items-center justify-between p-4 border border-gray-200 dark:border-gray-700 rounded-lg"
                    ...
                    >
                      <div className="flex items-center space-x-4">
                        ...
                        <div>
                          <div className="flex items-center gap-2">
                            <h3 className="text-lg font-medium text-gray-900 dark:text-white" data-testid={`review-repo-${review.id}`}>
                              {repo?.name || 'Unknown Repository'}
                            </h3>
                            <span className="text-gray-500" data-testid={`review-pr-${review.id}`}>
                          ...
                          <div className="flex items-center gap-4 mt-1 text-sm text-gray-600 dark:text-gray-400">
                            ...
                            {review.completedAt && (
                              <span data-testid={`review-completed-${review.id}`}>
                ...
                {mockReviewSessions.length === 0 && (
                  <div className="text-center py-8" data-testid="reviews-empty">
                    ...
                    <p className="text-gray-600 dark:text-gray-400">
                      Reviews will appear here once you create pull requests in connected repositories
                    </p>
                  </div>
                )}
              </div>
